...
> Agent Development Kit (ADK) agent** that anyone can install and run in two commands.
...
**Google Agent Development Kit (ADK for TypeScript, `@google/adk`)**. It's the part of
this repo I built specifically for the **Google for Startups AI Agents Challenge**, and
unlike the rest of the showcase it **runs on its own** — no monorepo, no private packages.

It grounds every answer in my **live** job corpus by calling the same public
`/api/jobs/semantic-search` endpoint my website uses (Vertex AI Search over ~20k
...
Both run on the same model layer and the same provider abstraction I show in this repo.

---

## The AI provider story

In production I run **TunzAI on Google Cloud Vertex AI**, served by my own **Gemini
model fine-tuned (SFT) on a proprietary ~21,000-example dataset** that encodes Tunzafy's
exact persona, tier rules, multilingual behavior, and safety guardrails. I grew that
corpus from an initial 14k to ~21k examples as I widened intent, tier, and language
coverage and tightened the guardrails.
...
1. **A runtime provider switch** ([`src/ai-provider/provider.ts`](src/ai-provider/provider.ts))
   that lets me serve every chat completion from my fine-tuned **Gemini/Vertex AI**
   model in production, while keeping **OpenAI wired in as an instant-rollback fallback** —
   selected by a single `AI_PROVIDER` env var with **zero changes at any call site**. Both
   paths return the identical OpenAI-shaped `ChatCompletion`, so live traffic runs on
   Gemini today and I can fail back in seconds if I ever need to.

2. **My dataset generator** ([`tuning/generate_training_data.mjs`](tuning/generate_training_data.mjs))
   that produces the fine-tuning corpus across every intent, all three tiers, and 31
   languages, plus a small [native Gemini-format sample](tuning/sample_native_tuning.jsonl)
   showing the `systemInstruction` / `contents` / `parts` schema I feed to Vertex AI tuning.
...
flowchart TD
    ...
    SW -->|GEMINI · live| VTX[Vertex AI · fine-tuned Gemini<br/>~21k-example SFT model]
    SW -->|OPENAI · fallback| OAI[OpenAI Chat Completions]
...
I built the provider abstraction as a strategy/factory pattern. My calling code keeps
...
In production I set `AI_PROVIDER=GEMINI`, so live traffic is served by my fine-tuned
Gemini endpoint; OpenAI stays in place purely as a fail-back. The Gemini branch lazily
loads `@google-cloud/vertexai`, maps OpenAI `messages` → Vertex `contents` +
`systemInstruction`, and normalizes the Vertex response back into the OpenAI
`ChatCompletion` shape — so all my existing parsing, error handling, and SSE streaming keep
working unchanged.
...
- **AI:** Google **Vertex AI** + my **fine-tuned Gemini (~21k-example SFT model)**, live in production; OpenAI kept only as an instant-rollback fallback; `@google-cloud/vertexai` SDK
...
I'm **Samuel Hatangimana** — I founded and build Tunzafy from Estonia.
